...
//
// focusStack convention: head (index 0) = MOST-recently focused = must receive the
// HIGHEST zIndex within its layer. We process oldest-first so each window gets a
// strictly-increasing counter, leaving the head with the largest value.
export function maybeCompact(state: ManagerState): ManagerState {
  ...
  // Index each visible window by its position in the focus stack (head = most-recent).
  ...
  let normalCounter = 0;
  let pinnedCounter = 0;
  // Sort oldest-first (highest rank first) so they are assigned the lowest counters,
  // and the head (rank 0) ends up with the highest counter — preserving stacking.
  const sorted = [...visible].sort((a, b) => rank(b.id) - rank(a.id));
